...
public class Assert extends org.junit.Assert {
	...
	public static <T> void assertContains(Collection<T> collection, T element) {
		assertTrue(//
			String.format(
				"collection %s should contain element %s",
				collection,
				element),
			collection.contains(element));
	}

	public static <T> void assertContains(String message,
		Collection<T> collection, T element) {
		assertTrue(message, collection.contains(element));
	}

	public static <K, V> void assertContains(String message, Map<K, V> map,
		K key, V value) {
		assertEquals(message, value, map.get(key));
	}

	public static <K, V> void assertContainsEntry(String message, Map<K, V> map,
		K key, V value) {
		assertEquals(message, value, map.get(key));
	}
	...
	public static <K, V> void assertContainsEntry(Map<K, V> map, K key, V value) {
		assertEquals( //
			String.format(
				"map %s should contain key %s and value %s",
				map,
				key,
				value),
			value,
